...
async function addNewThread(e) {
    ...
    try {
        const docId = await FirestoreController.addNewThread(thread);
        thread.set_docId(docId);
        // home_page(); // will be improved later
        ...
        // attach event listener to the new thread forms
...
export function buildHomeScreen(threadList) {
    ...
    if (threadList.length == 0) {
        html += `<h4 id="no-thread-found">No Threads Found`
    }

    Elements.root.innerHTML = html;
      // attach event listeners to view button
    ThreadPage.addViewFormEvent()
}
